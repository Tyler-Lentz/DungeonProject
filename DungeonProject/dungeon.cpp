...
void UnderwaterTemple::makeFloor4(std::mutex& roomMut)
{
    unsigned int tfloor = 4;
    {
        std::vector<std::string> roomTemplate;
        ...
        roomTemplate.push_back("#################");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(4, 11), new Key(pgame, Coordinate(4, 11)));
    ...
    }
    {
        ...
        roomTemplate.push_back("#################");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(1, 1), new Potion(pgame, Coordinate(1, 1), dngutil::POTION_HEAL));
        ...
        int difficulty = 2;
        int backColor = dngutil::LIGHTBLUE;
        std::string name = "Crumbling Roof";
        Coordinate mapCoord(0, 1);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, nullptr));
    ...
    }
    {
        ...
        auto puzzleAction = [this](std::list<Creature*> creatureList, GAMEMAP& gameMap) -> void
        {
            gameMap[2][8].push_back(new ExitObject(pgame, Coordinate(1, 8), false, ColorChar('v', dngutil::WHITE)));
        };
        ...
        int difficulty = 2;
        int backColor = dngutil::LIGHTBLUE;
        std::string name = "Hidden Staircase";
        Coordinate mapCoord(0, -1);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, new Puzzle(puzzleSolved, puzzleAction)));
    ...
    }
    {
        ...
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(9, 1), new Potion(pgame, Coordinate(9, 1), dngutil::POTION_HEAL));
    ...
    }
    {
        ...
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(3, 9), new Potion(pgame, Coordinate(3, 9), dngutil::POTION_HEAL));
    ...
    }
    {
        ...
        {
            std::list<Coordinate> deletionList;
            deletionList.push_back(Coordinate(9, 1));
            deletionList.push_back(Coordinate(10, 1));
...
void UnderwaterTemple::makeFloor3(std::mutex& roomMut)
{
    ...
    }
    {
        ...
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(1, 2), new Potion(pgame, Coordinate(1, 2), dngutil::POTION_HEAL));
        specificObjects.emplace(Coordinate(1, 3), new Potion(pgame, Coordinate(1, 3), dngutil::POTION_HEAL));

        std::vector<dngutil::TID> possibleCreatures;

        int difficulty = 0;
        int backColor = dngutil::LIGHTBLUE;
        std::string name = "Potion Room";
        Coordinate mapCoord(-2, -1);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, nullptr));
    ...
    }
    {
        ...
        auto puzzleAction = [this, game](std::list<Creature*> creatureList, GAMEMAP& gameMap) -> void
        {
            ...
            game->getVWin()->txtmacs.clearMapArea(false, NULL);
            for (auto i : deletionList)
            {
                WallObject* wall = dynamic_cast<WallObject*>(gameMap[i.y][i.x].back());
        ...
        int difficulty = 9;
        int backColor = dngutil::LIGHTBLUE;
        std::string name = "Enchanted Walls";
        Coordinate mapCoord(1, -1);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, new Puzzle(puzzleSolved, puzzleAction)));
...
void UnderwaterTemple::makeFloor2(std::mutex& roomMut)
{
    ...
    }
    {
        ...
        roomTemplate.push_back("###########");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(9, 5), new MagicalPotion(pgame, Coordinate(9, 5)));
        specificObjects.emplace(Coordinate(1, 11), new Secondary(
            pgame,
    ...
    }
    {
        ...
        int difficulty = 10;
        int backColor = dngutil::LIGHTBLUE;
        std::string name = "Water Horse's Blockade";
        Coordinate mapCoord(0, 0);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, nullptr));
    ...
    }
    {
        ...
        roomTemplate.push_back("#################");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(15, 2), new Key(pgame, Coordinate(15, 2)));
...
void UnderwaterTemple::makeFloor1(std::mutex& roomMut)
{
    unsigned int tfloor = 1;
    {
        std::vector<std::string> roomTemplate;
        ...
        roomTemplate.push_back("###########");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(9, 3), new MagicalPotion(pgame, Coordinate(9, 3)));

        std::vector<dngutil::TID> possibleCreatures;

        int difficulty = 10;
        int backColor = dngutil::BLUE;
        std::string name = "Beyond";
        Coordinate mapCoord(0, 0);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, nullptr));
    ...
    }
    {
        ...
        roomTemplate.push_back("#      #");
        roomTemplate.push_back("# #  # #");
        roomTemplate.push_back("#o#oo#o#");
        roomTemplate.push_back("########");
        Game* game = pgame;
        auto puzzleSolved = [game](const std::list<Creature*>& creatureList, const GAMEMAP& gameMap) -> bool
        {
            if (game->getPlayer()->getCoord().y == 8)
            {
                return true;
            }
            return false;
        ...
        auto puzzleAction = [this, game](std::list<Creature*> creatureList, GAMEMAP& gameMap) -> void
        {
            int playerX = game->getPlayer()->getCoord().x;
            int y = game->getPlayer()->getCoord().y;
            if (playerX == 1)
            {
                gameMap[y][3].push_back(new WallObject(game, Coordinate(3, y)));
                gameMap[y][4].push_back(new WallObject(game, Coordinate(4, y)));
                gameMap[y][6].push_back(new WallObject(game, Coordinate(6, y)));
            }
            else if (playerX == 3 || playerX == 4)
            {
                gameMap[y][1].push_back(new WallObject(game, Coordinate(1, y)));
                gameMap[y][6].push_back(new WallObject(game, Coordinate(6, y)));
            }
            else if (playerX == 6)
            {
        ...
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(1, 9), new Secondary(
            pgame,
            ...
            300,
            1,
            "With great focus you can block many attacks"
        ));
        specificObjects.emplace(Coordinate(3, 9), new Potion(pgame, Coordinate(3, 9), dngutil::POTION_HEAL + 30));
        specificObjects.emplace(Coordinate(4, 9), new Potion(pgame, Coordinate(4, 9), dngutil::POTION_HEAL + 30));
        specificObjects.emplace(Coordinate(6, 9), new Flute(pgame, Coordinate(6, 9)));

        std::vector<dngutil::TID> possibleCreatures;

        int difficulty = 10;
        int backColor = dngutil::BLUE;
        std::string name = "One Choice";
        Coordinate mapCoord(-1, 0);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, new Puzzle(puzzleSolved, puzzleAction)));
    ...
    }
    {
        ...
        roomTemplate.push_back("######");
        std::map<Coordinate, MapObject*> specificObjects;
        specificObjects.emplace(Coordinate(1, 11), new Potion(pgame, Coordinate(1, 11), dngutil::POTION_HEAL + 35));

        std::vector<dngutil::TID> possibleCreatures;

        int difficulty = 10;
        int backColor = dngutil::BLUE;
        std::string name = "Bloodjaw's Stairwell";
        Coordinate mapCoord(1, -1);
        RoomInfo rminfo(roomTemplate, specificObjects, name, difficulty, backColor, possibleCreatures, tfloor, mapCoord);
        roomMut.lock();
        gamespace[tfloor].emplace(mapCoord, new Room(pgame, rminfo, nullptr));
